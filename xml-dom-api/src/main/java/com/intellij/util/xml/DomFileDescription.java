...
public class DomFileDescription<T> {
    ...
    private final Map<Class<? super ScopeProvider>, ScopeProvider> myScopeProviders = ConcurrentFactoryMap.createMap(aClass -> (ScopeProvider) Application.get().getUnbindedInstance(aClass));
    ...
    public final void registerNamespacePolicy(@Nonnull String namespaceKey, final String... namespaces) {
        registerNamespacePolicy(namespaceKey, xmlTag -> Arrays.asList(namespaces));
    }
     ...
     * {@link com.intellij.util.xml.DomService#getDomFileCandidates(Class, Project)} index is rebuilt correctly.
     */
