...
public class XmlHighlightVisitor extends XmlElementVisitor implements HighlightVisitor, IdeValidationHost
{
	...
	@RequiredReadAction
	private void bindMessageToAstNode(final PsiElement childByRole, final HighlightInfoType warning, final int offset, int length, @NotNull String localizedMessage,
			IntentionAction... quickFixActions)
	{
		if(childByRole != null)
		{
			...
			{
				highlightInfo = HighlightInfo.newHighlightInfo(warning).range(new TextRange(startOffset, startOffset + length)).textAttributes(NONEMPTY_TEXT_ATTRIBUTES).descriptionAndTooltip
			...
			final IntentionAction addAttributeFix)
	{
		...
		{
			addElementsForTagWithManyQuickFixes(tag, localizedMessage, isInjectedHtmlTagWithoutValidation((HtmlTag) tag) ? HighlightInfoType.INFORMATION : SeverityRegistrar.getSeverityRegistrar(tag
					.getProject()).getHighlightInfoTypeBySeverity(profile.getErrorLevel(key, tag).getSeverity()), addAttributeFix, basicIntention);
		}
		else if(!htmlTag)
		{
	...
	private static HighlightInfoType getTagProblemInfoType(XmlTag tag)
	{
		...
		{
			if(isInjectedHtmlTagWithoutValidation((HtmlTag) tag))
			{
	...
	private void checkDuplicateAttribute(XmlTag tag, final XmlAttribute attribute)
	{
		...
		for(XmlAttribute tagAttribute : attributes)
		{
			...
			if(attribute != tagAttribute && Comparing.strEqual(attribute.getName(), tagAttribute.getName()))
			{
				...
				HighlightInfo highlightInfo = HighlightInfo.newHighlightInfo(getTagProblemInfoType(tag)).range(attributeNameNode).descriptionAndTooltip(XmlErrorMessages.message("duplicate" +
						".attribute", localName)).create();
				addToResults(highlightInfo);
