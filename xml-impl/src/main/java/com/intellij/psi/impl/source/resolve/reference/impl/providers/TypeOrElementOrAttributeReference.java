...
public class TypeOrElementOrAttributeReference implements PsiReference
{
	...
	{
		return ResolveCache.getInstance(getElement().getProject()).resolveWithCaching(this, (ref, incompleteCode) -> ref.resolveInner(), false, false);
	}
	...
	private PsiElement resolveInner()
	{
		...
		if(myType != null && nsDescriptor != null && nsDescriptor.getTag() != null)
		{
			...
			switch(myType)
			{
				...
				{
					XmlElementDescriptor descriptor = nsDescriptor.getElementDescriptor(XmlUtil.findLocalNameByQualifiedName(canonicalText), getNamespace(tag, canonicalText),
							new HashSet<XmlNSDescriptorImpl>(), true);
	...
	XmlNSDescriptorImpl getDescriptor(final XmlTag tag, String text)
	{
		if(myType != ReferenceType.ElementReference && myType != ReferenceType.AttributeReference)
		{
			...
			if(parentElement instanceof XmlAttribute && grandParentElement instanceof XmlTag)
			{
				...
				doRedefineCheck = (SchemaReferencesProvider.REF_ATTR_NAME.equals(attrName) && (SchemaReferencesProvider.GROUP_TAG_NAME.equals(tagLocalName) || SchemaReferencesProvider
						.ATTRIBUTE_GROUP_TAG_NAME.equals(tagLocalName))) || (SchemaReferencesProvider.BASE_ATTR_NAME.equals(attrName) || SchemaReferencesProvider.MEMBER_TYPES_ATTR_NAME.equals
						(attrName));
			}
	...
	public PsiElement handleElementRename(String newElementName) throws IncorrectOperationException
	{
		...
		final PsiElement element = ElementManipulators.getManipulator(myElement).handleContentChange(myElement, getRangeInElement(), newElementName);
	...
	private static void processNamespace(final String namespace, final CompletionProcessor processor, final XmlNSDescriptorImpl nsDescriptor, final String[] tagNames)
	{
		processor.namespace = namespace;

		XmlNSDescriptorImpl.processTagsInNamespace(nsDescriptor.getTag(), tagNames, processor);
	}
