...
public class XmlNSDescriptorImpl implements XmlNSDescriptorEx, Validator<XmlDocument>, DumbAware, XmlNSTypeDescriptorProvider {
    ...
    private final Map<QNameKey, CachedValue<XmlElementDescriptor>> myDescriptorsMap =
        Collections.synchronizedMap(new HashMap<QNameKey, CachedValue<XmlElementDescriptor>>());
    private final Map<Pair<QNameKey, XmlTag>, CachedValue<TypeDescriptor>> myTypesMap =
        Collections.synchronizedMap(new HashMap<Pair<QNameKey, XmlTag>, CachedValue<TypeDescriptor>>());
            ...
            while (tokenizer.hasMoreTokens()) {
                ...
                if (tokenizer.hasMoreTokens()) {
                    PsiFile resourceLocation =
                        ExternalResourceManager.getInstance().getResourceLocation(tokenizer.nextToken(), myFile, null);
    ...
    public static boolean checkSchemaNamespace(String namespace) {
        return XmlUtil.XML_SCHEMA_URI.equals(namespace) || XmlUtil.XML_SCHEMA_URI2.equals(namespace) || XmlUtil.XML_SCHEMA_URI3.equals(
            namespace);
    }
        ...
        for (XmlTag tag : tags) {
            for (String tagName : tagNames) {
                if (equalsToSchemaName(tag, tagName)) {
                    ...
                    if (name != null && !processor.execute(tag)) {
                        return false;
                    }
                ...
                if (schemaLocation != null) {
                    ...
                    if (xmlFile != null) {
                        ...
                        if (includedDocument != null
                            && !processTagsInNamespaceInner(includedDocument.getRootTag(), tagNames, processor, visitedTags)) {
                            return false;
                        }
                    }
                }
        ...
        for (XmlTag tag : tags) {
            if (equalsToSchemaName(tag, specialName)) {
                ...
                equalsToSchemaName(tag, IMPORT_TAG_NAME)
                    && rootTag.getNamespaceByPrefix(XmlUtil.findPrefixByQualifiedName(name))
    ...
    public static XmlNSDescriptorImpl getRedefinedElementDescriptor(final XmlTag parentTag) {
        ...
        if (file != null) {
            ...
            if (metaData instanceof XmlNSDescriptorImpl xmlNSDescriptor) {
                return xmlNSDescriptor;
            }
        }
        return null;
    ...
    public static XmlFile getRedefinedElementDescriptorFile(final XmlTag parentTag) {
        ...
        if (schemaL != null) {
            final PsiReference[] references =
                parentTag.getAttribute(XmlUtil.SCHEMA_LOCATION_ATT, null).getValueElement().getReferences();
    ...
    public XmlElementDescriptor getElementDescriptor(
        ...
        for (final XmlTag tag : tags) {
                ...
                if (name != null) {
                    if (checkElementNameEquivalence(localName, namespace, name, tag)) {
                        final CachedValue<XmlElementDescriptor> cachedValue =
                            CachedValuesManager.getManager(tag.getProject()).createCachedValue(() ->
                            {
            ...
            else if (equalsToSchemaName(tag, INCLUDE_TAG_NAME) || (reference && equalsToSchemaName(
                tag,
                IMPORT_TAG_NAME
            ) && (namespace.equals(tag.getAttributeValue("namespace")) || namespace.length()
                == 0 && tag.getAttributeValue("namespace") == null))) {
                final String schemaLocation = tag.getAttributeValue("schemaLocation");
                if (schemaLocation != null) {
                    final XmlFile xmlFile = XmlUtil.findNamespace(rootTag.getContainingFile(), schemaLocation);
                    if (xmlFile != null) {
    ...
    private boolean checkElementNameEquivalence(String localName, String namespace, String fqn, XmlTag context) {
        ...
        if (myTargetNamespace == null) {
            ...
            if (myTargetNamespace.equals(namespace)) {
                return true;
            }
    ...
    private XmlAttributeDescriptor getAttributeImpl(String localName, String namespace, @Nullable Set<XmlTag> visited) {
        ...
        for (XmlTag tag : tags) {
            ...
            else if (equalsToSchemaName(tag, INCLUDE_TAG_NAME) || (equalsToSchemaName(
                tag,
                IMPORT_TAG_NAME
                            ...
                            if (data instanceof XmlNSDescriptorImpl) {
                                ...
                                if (attributeDescriptor != null) {
                                    final CachedValue<XmlAttributeDescriptor> value =
                                        CachedValuesManager.getManager(includedDocument.getProject()).createCachedValue(() ->
                                        {
                            ...
                            final CachedValue<TypeDescriptor> value =
                                CachedValuesManager.getManager(tag.getProject()).createCachedValue(() ->
                                {
                                    ...
                                    if ((currentName != null && !currentName.equals(XmlUtil.findLocalNameByQualifiedName(name)))
                                        || !xmlFile.isValid() || xmlFile.getDocument() == null) {
    ...
    private XmlNSDescriptorImpl findNSDescriptor(final XmlTag tag, final XmlDocument document) {
        ...
        if (IMPORT_TAG_NAME.equals(tag.getLocalName())) {
            ...
            nsDescriptor = (importedDescriptor instanceof XmlNSDescriptorImpl xmlNSDescriptor) ? xmlNSDescriptor : this;
        }
        else {
            nsDescriptor = this;
        }
    ...
    public Object[] getDependences() {
        if (dependencies == null) {
            dependencies = myFile == null ? ArrayUtil.EMPTY_OBJECT_ARRAY : new Object[]{myFile}; // init was not called
        }
        return dependencies;
    }
