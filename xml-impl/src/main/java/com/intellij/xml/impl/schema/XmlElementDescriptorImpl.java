...
public class XmlElementDescriptorImpl extends XsdEnumerationDescriptor<XmlTag> implements XmlElementDescriptor, PsiWritableMetaData, Validator<XmlTag>, XmlElementDescriptorAwareAboutChildren {
    ...
    if (context instanceof XmlElement) {
      ...
      if (tag != null) {
        ...
        if (namespacePrefix != null && namespacePrefix.length() > 0) {
          ...
          if (rootTag != null && (NONQUALIFIED_ATTR_VALUE.equals(elementFormDefault = rootTag.getAttributeValue(ELEMENT_FORM_DEFAULT)) || elementFormDefault == null /*unqualified is
  ...
  public XmlElementDescriptor[] getElementsDescriptors(XmlTag context) {
    ...
    if (type instanceof ComplexTypeDescriptor) {
      ...
      String contextNs;
      if (context != null && descriptor.canContainTag(context.getLocalName(), contextNs = context.getNamespace(), context) && (!contextNs.equals(getNamespace()) || descriptor
        ...
        if (nsDescriptor != null) {
          elementsDescriptors = ArrayUtil.mergeArrays(elementsDescriptors, nsDescriptor.getRootElementsDescriptors(((XmlFile) containingFile).getDocument()));
        }
      }
    }
  ...
  protected XmlElementDescriptor getElementDescriptor(final String localName, final String namespace, XmlElement context, String fullName) {
    ...
    for (XmlElementDescriptor element1 : elements) {
      ...
      if (element.getName().equals(localName)) {
        if (namespace == null || namespace.equals(namespaceByContext) || namespaceByContext.equals(XmlUtil.EMPTY_URI) || element.getName(context).equals(fullName) || (namespace.length() == 0)
