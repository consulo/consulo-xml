...
public class XmlElementDescriptorImpl extends XsdEnumerationDescriptor<XmlTag>
        ...
        if (context instanceof XmlElement) {
            ...
            if (tag != null) {
                ...
                if (namespacePrefix != null && namespacePrefix.length() > 0) {
                    ...
                    if (rootTag != null && (NONQUALIFIED_ATTR_VALUE.equals(
                        elementFormDefault = rootTag.getAttributeValue(ELEMENT_FORM_DEFAULT)) || elementFormDefault == null /*unqualified is
          default*/) && tag.getNamespaceByPrefix("").isEmpty() && myDescriptorTag.getParentTag() != rootTag) {
    ...
    public String getNamespace() {
        ...
        if (getNSDescriptor() == null || myDescriptorTag == null) {
            return XmlUtil.EMPTY_URI;
        }
        final String namespacePrefix = XmlUtil.findPrefixByQualifiedName(name);
        return namespacePrefix.isEmpty() ? getDefaultNamespace() : myDescriptorTag.getNamespaceByPrefix(namespacePrefix);
    }
    ...
    private XmlNSDescriptor getNSDescriptor(XmlElement context) {
        ...
        if (context instanceof XmlTag && nsDescriptor instanceof XmlNSDescriptorImpl) {
            ...
            if (XmlUtil.XML_SCHEMA_URI.equals(defaultNamespace)) {
                return nsDescriptor; // do not check for overriden for efficiency
            }
        ...
        if (type == null) {
            ...
            if (substAttr != null) {
                ...
                final String namespace =
                    namespacePrefix.isEmpty() ? getDefaultNamespace() : myDescriptorTag.getNamespaceByPrefix(namespacePrefix);
                final String local = XmlUtil.findLocalNameByQualifiedName(substAttr);
                final XmlElementDescriptorImpl originalElement =
                    (XmlElementDescriptorImpl)((XmlNSDescriptorImpl)getNSDescriptor()).getElementDescriptor(local, namespace);
    ...
    public XmlElementDescriptor[] getElementsDescriptors(XmlTag context) {
        ...
        if (type instanceof ComplexTypeDescriptor) {
            ...
            String contextNs;
            if (context != null && descriptor.canContainTag(
                context.getLocalName(),
                contextNs = context.getNamespace(),
                context
            ) && (!contextNs.equals(getNamespace()) || descriptor
                .hasAnyInContentModel()) && containingFile instanceof XmlFile) { // JSXmlLiteralExpressionImpl, being an xml element itself, may be contained in non-XML file
            ...
            if (context != null) {
                ...
                if (!seenXmlNs) {
                    attributeDescriptors =
                        updateAttributeDescriptorsFromAny(context, typeDescriptor, attributeDescriptors, XmlUtil.XML_NAMESPACE_URI);
                }
            }
    ...
    private XmlAttributeDescriptor getAttributeDescriptorImpl(final String attributeName, XmlTag context) {
        ...
        final String namespace = namespacePrefix.isEmpty() ? getDefaultNamespace() : context.getNamespaceByPrefix(namespacePrefix);
    ...
    private String getDefaultNamespace() {
        XmlNSDescriptor nsDescriptor = getNSDescriptor();
        return nsDescriptor instanceof XmlNSDescriptorImpl xmlNSDescriptor ? xmlNSDescriptor.getDefaultNamespace() : "";
    }
    ...
    public XmlElementDescriptor getElementDescriptor(final String name) {
        ...
        final String namespacePrefix = XmlUtil.findPrefixByQualifiedName(name);
        final String namespace = namespacePrefix.isEmpty() ? getDefaultNamespace() : myDescriptorTag.getNamespaceByPrefix(namespacePrefix);
        return getElementDescriptor(localName, namespace, null, name);
    }

    @Nullable
    protected XmlElementDescriptor getElementDescriptor(
        ...
        for (XmlElementDescriptor element1 : elements) {
            ...
            if (element.getName().equals(localName)) {
                if (namespace == null || namespace.equals(namespaceByContext)
                    || namespaceByContext.equals(XmlUtil.EMPTY_URI) || element.getName(context).equals(fullName)
                    || namespace.isEmpty() && element.getDefaultName().equals(fullName)) {
                    return element;
                }
                else {
                    final XmlNSDescriptor descriptor =
                        context instanceof XmlTag ? ((XmlTag)context).getNSDescriptor(namespace, true) : null;

                    // schema's targetNamespace could be different from file systemId used as NS
                    if (descriptor instanceof XmlNSDescriptorImpl xmlNSDescriptor) {
                        if (xmlNSDescriptor.getDefaultNamespace().equals(namespaceByContext)) {
                            return element;
                        }
                        else {
                            xmlNSDescriptor.getSubstitutes(localName, namespace);
                        }
                    }
                }
            }
    ...
    public XmlElementDescriptor getElementDescriptor(XmlTag element, XmlTag contextTag) {
        ...
        XmlElementDescriptor elementDescriptor =
            getElementDescriptor(element.getLocalName(), element.getNamespace(), context, element.getName());
        ...
        if (elementDescriptor == null || element.getAttributeValue("xsi:type") != null) {
            ...
            else if (context instanceof XmlTag tag && tag.getAttributeValue("xsi:type") != null && askParentDescriptorViaXsi()) {
                final XmlElementDescriptor parentXmlDescriptorByType = XmlUtil.findXmlDescriptorByType(tag);
    ...
    public String getDefaultName() {
        ...
        XmlTag rootTag = psiFile instanceof XmlFile xmlFile ? xmlFile.getRootTag() : null;
    ...
    public boolean equals(Object o) {
        ...
        if (myDescriptorTag != null ? !myDescriptorTag.equals(that.myDescriptorTag) : that.myDescriptorTag != null) {
            return false;
        }
