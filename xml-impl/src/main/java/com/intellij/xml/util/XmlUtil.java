...
public class XmlUtil {
    ...
    static {
        final URL xhtml4SchemaLocationUrl =
            XmlUtil.class.getResource(ExternalResourceManagerEx.STANDARD_SCHEMAS + "xhtml1-transitional.xsd");
        XHTML4_SCHEMA_LOCATION =
            VirtualFileUtil.urlToPath(VirtualFileUtil.toIdeaUrl(FileUtil.unquote(xhtml4SchemaLocationUrl.toExternalForm()), false));
    }
    ...
    public static String findNamespacePrefixByURI(XmlFile file, String uri) {
        ...
        for (XmlAttribute attribute : tag.getAttributes()) {
            ...
            if ("xmlns".equals(attribute.getName()) && uri.equals(attribute.getValue())) {
                return "";
            }
        }
    ...
    public static String[] findNamespacesByURI(XmlFile file, String uri) {
        ...
        for (XmlAttribute attribute : attributes) {
            ...
            if ("xmlns".equals(attribute.getName()) && attribute.getValue().equals(uri)) {
                result.add("");
            }
        }
    ...
    @Nullable
    public static XmlFile findNamespace(PsiFile base, String nsLocation) {
    ...
    @Nullable
    public static XmlFile findNamespaceByLocation(PsiFile base, String nsLocation) {
        final String location = ExternalResourceManager.getInstance().getResourceLocation(nsLocation, base.getProject());
        return findXmlFile(base, location);
    }
    ...
    public static boolean processXmlElements(
    ...
    public static boolean tagFromTemplateFramework(final XmlTag tag) {
    ...
    public static PsiFile findRelativeFile(String uri, PsiElement base) {
        if (base instanceof PsiFile) {
            ...
            return file == null ? null : base.getManager().findFile(file);
        }
    ...
    public static String getCommentText(XmlComment comment) {
        ...
        return firstChild != null && firstChild.getNextSibling() instanceof XmlToken token
            && token.getTokenType() == XmlTokenType.XML_COMMENT_CHARACTERS
            ? token.getText()
            : null;
    }
    ...
    public static void expandTag(XmlTag tag) {
        XmlTag newTag =
            XmlElementFactory.getInstance(tag.getProject()).createTagFromText('<' + tag.getName() + "></" + tag.getName() + '>');
     ...
     */
    public static XmlTag addChildTag(XmlTag parent, XmlTag child) throws consulo.language.util.IncorrectOperationException {
        return addChildTag(parent, child, -1);
    }

    public static XmlTag addChildTag(XmlTag parent, XmlTag child, int index) throws consulo.language.util.IncorrectOperationException {
  ...
  /*tag.processElements(new FilterElementProcessor(XmlTagFilter.INSTANCE) {
     ...
     * @return
     */
    public static XmlTag createChildTag(
        final XmlTag xmlTag,
        String localName,
        String namespace,
        ...
        try {
            String tagStart =
                qname + (
                    !StringUtil.isEmpty(namespace) && xmlTag.getPrefixByNamespace(namespace) == null
    ...
    public static Pair<XmlTagChild, XmlTagChild> findTagChildrenInRange(final PsiFile file, int startOffset, int endOffset) {
        ...
        if (first.getTextRange().getStartOffset() != startOffset) {
            ...
            if (!(elementAt instanceof PsiWhiteSpace) || elementAt.getTextRange().getEndOffset() != startOffset) {
                return null;
            }
        }
    ...
    public static boolean isSimpleValue(final String unquotedValue, final PsiElement context) {
    ...
    public static String generateDocumentDTD(XmlDocument doc, boolean full) {
        ...
        try {
            ...
            PsiElement firstSibling = rootTag != null ? rootTag.getNextSibling() : null;
            for (PsiElement element = firstSibling; element != null; element = element.getNextSibling()) {
                if (element instanceof XmlTag tag) {
                    computeTag(tag, tags, attributes, full);
                }
            }
        }
        finally {
    ...
    public static XmlFile getContainingFile(PsiElement element) {
        while (!(element instanceof XmlFile) && element != null) {
            ...
            if (context == null) {
                // todo Dmitry Avdeev: either XmlExtension should work on any PsiFile (not just XmlFile),
                // or you need to handle elements from JspJavaFile in some other way
    ...
    public static XmlElement setNewValue(XmlElement tag, String value) throws IncorrectOperationException {
        ...
        else if (tag instanceof XmlAttribute attr) {
            attr.setValue(value);
            return attr;
        }
        else {
    ...
    public static void registerXmlAttributeValueReferenceProvider(
    ...
    public static void registerXmlAttributeValueReferenceProvider(
        ...
        registrar.registerReferenceProvider(XmlPatterns.xmlAttributeValue()
            .withLocalName(namePattern)
            .and(new FilterPattern(elementFilter)), provider, priority);
    }

    public static void registerXmlTagReferenceProvider(
        PsiReferenceRegistrar registrar,
        String[] names,
        @Nullable ElementFilter elementFilter,
        boolean caseSensitive,
        ...
        if (names == null) {
            registrar.registerReferenceProvider(
                XmlPatterns.xmlTag().and(new FilterPattern(elementFilter)),
                provider,
                PsiReferenceRegistrar.DEFAULT_PRIORITY
            );
            return;
        ...
        final StringPattern namePattern = caseSensitive
            ? StandardPatterns.string().oneOf(names)
            : StandardPatterns.string().oneOfIgnoreCase(names);
        registrar.registerReferenceProvider(
            XmlPatterns.xmlTag().withLocalName(namePattern).and(new FilterPattern(elementFilter)),
            provider,
            PsiReferenceRegistrar.DEFAULT_PRIORITY
        );
    }
    ...
    public static XmlFile findDescriptorFile(XmlTag tag, XmlFile containingFile) {
        ...
        XmlFile descriptorFile = nsDescriptor != null
            ...
            : containingFile.getDocument().getProlog().getDoctype() != null
            ? containingFile
            : null;
