...
public class XsdComplexTypeInfoBuilder extends NanoXmlUtil.IXMLBuilderAdapter {
    ...
    private static final Logger LOG = Logger.getInstance("#XsdComplexTypeInfoBuilder");
    ...
    public void addAttribute(String key, String nsPrefix, String nsURI, String value, String type) throws Exception {
        if (!StringUtil.isEmptyOrSpaces(nsURI) && !HTTP_WWW_W3_ORG_2001_XMLSCHEMA.equals(nsURI)) {
            return;
        }
        if ("base".equals(key)) {
            ...
            if (SIGN.equals(myCurrentElementName) && !myInsideContent && !myInsideExtension && !myInsideRestriction
                && !myInsideSchema && myCurrentComplexTypeName == null && myCurrentSimpleTypeName == null) {
                myCurrentElementName = value;
            }
            else if (SIGN.equals(myCurrentComplexTypeName) && !myInsideContent && !myInsideExtension
                && !myInsideRestriction && !myInsideSchema && myCurrentSimpleTypeName == null) {
                myCurrentComplexTypeName = value;
            }
            else if (SIGN.equals(myCurrentSimpleTypeName) && !myInsideContent && !myInsideExtension
                && !myInsideRestriction && !myInsideSchema && myCurrentComplexTypeName == null) {
