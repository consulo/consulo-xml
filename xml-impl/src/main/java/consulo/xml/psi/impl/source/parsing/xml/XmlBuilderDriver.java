...
public class XmlBuilderDriver {
    ...
    public void build(XmlBuilder builder) {
        ...
        SimpleReference<LighterASTNode[]> childrenRef = SimpleReference.create(null);
        int count = structure.getChildren(root, childrenRef);
    ...
    private void processPrologNode(
        ...
        SimpleReference<LighterASTNode[]> prologChildren = new SimpleReference<>(null);
        int prologChildrenCount = structure.getChildren(structure.prepareForGetChildren(prolog), prologChildren);
    ...
    private void processDoctypeNode(
        XmlBuilder builder,
        FlyweightCapableTreeStructure<LighterASTNode> structure,
        LighterASTNode doctype
    ) {
        SimpleReference<LighterASTNode[]> tokens = new SimpleReference<>(null);
        int tokenCount = structure.getChildren(structure.prepareForGetChildren(doctype), tokens);
        if (tokenCount > 0) {
    ...
    protected PsiBuilder createBuilderAndParse() {
        ParserDefinition xmlParserDefinition = ParserDefinition.forLanguage(XMLLanguage.INSTANCE);
        ...
        PsiBuilder b = PsiBuilderFactory.getInstance().createBuilder(
            xmlParserDefinition,
            xmlParserDefinition.createLexer(defaultVersion),
            defaultVersion,
            myText
        );
    ...
    private void processErrorNode(PsiBuilder psiBuilder, LighterASTNode node, XmlBuilder builder) {
        ...
        LocalizeValue message = psiBuilder.getErrorMessage(node);
        assert message != null;
    ...
    private void processTagNode(
        ...
        SimpleReference<LighterASTNode[]> childrenRef = SimpleReference.create(null);
        int count = structure.getChildren(node, childrenRef);
        LighterASTNode[] children = childrenRef.get();
        ...
        XmlBuilder.ProcessingOrder order =
            builder.startTag(localName, namespace, node.getStartOffset(), node.getEndOffset(), headerEndOffset);
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (tt == TokenType.ERROR_ELEMENT) {
                processErrorNode(psiBuilder, child, builder);
            }
            if (tt == XmlElementType.XML_TAG || tt == XmlElementType.HTML_TAG) {
                processTagNode(psiBuilder, structure, child, builder);
            }
            if (processAttrs && tt == XmlElementType.XML_ATTRIBUTE) {
                processAttributeNode(child, structure, builder);
            }
            if (processTexts && tt == XmlElementType.XML_TEXT) {
                processTextNode(structure, child, builder);
            }
            if (tt == XmlElementType.XML_ENTITY_REF) {
                builder.entityRef(getTokenText(child), child.getStartOffset(), child.getEndOffset());
            }
        }
    ...
    private void processTextNode(FlyweightCapableTreeStructure<LighterASTNode> structure, LighterASTNode node, XmlBuilder builder) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            CharSequence physical = getTokenText(child);

            if (XmlTokenType.COMMENTS.contains(tt)) {
                continue;
            }
    ...
    private void processAttributeNode(
        LighterASTNode attrNode,
        FlyweightCapableTreeStructure<LighterASTNode> structure,
        XmlBuilder builder
    ) {
        builder.attribute(
    ...
    private String getNamespace(CharSequence tagName) {
        ...
        for (int i = myPrefixesStack.size() - 1; i >= 0; i--) {
            if (namespacePrefix.equals(myPrefixesStack.get(i))) {
                return myNamespacesStack.get(i);
            }
        }
    ...
    private void checkForXmlns(LighterASTNode attrNode, FlyweightCapableTreeStructure<LighterASTNode> structure) {
        CharSequence name = getAttributeName(attrNode, structure);
    ...
    private CharSequence getAttributeValue(LighterASTNode attrNode, FlyweightCapableTreeStructure<LighterASTNode> structure) {
        CharSequence fullValue = findTextByTokenType(attrNode, structure, XmlElementType.XML_ATTRIBUTE_VALUE);
        int start = 0;
    ...
    private CharSequence findTextByTokenType(
        LighterASTNode attrNode,
        FlyweightCapableTreeStructure<LighterASTNode> structure,
        IElementType tt
    ) {
        ...
        SimpleReference<LighterASTNode[]> childrenRef = SimpleReference.create(null);
        int count = structure.getChildren(attrNode, childrenRef);
        LighterASTNode[] children = childrenRef.get();
