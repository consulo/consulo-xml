...
public class XmlTagImpl extends XmlElementImpl implements XmlTag, HintedReferenceHost {
  ...
  private Map<String, CachedValue<XmlNSDescriptor>> initializeSchema(@Nonnull final String namespace,
    ...
    map.put(namespace, CachedValuesManager.getManager(getManager().getProject()).createCachedValue(() ->
    {
            ...
            if (elementDescriptor != null) {
              ...
              if (attributeDescriptor != null && attributeDescriptor.isFixed()) {
                ...
                if (defaultValue != null && defaultValue.equals(namespace)) {
                  return new CachedValueProvider.Result<>(descriptor, descriptor.getDependences(), this, ExternalResourceManager.getInstance());
                }
              }
            }
          }
      ...
      return new CachedValueProvider.Result<>(null, this, currentFile == null ? this : currentFile, ExternalResourceManager.getInstance());
    }, false));
  ...
  public XmlAttribute getAttribute(String name, String namespace) {
    if (name != null && name.indexOf(':') != -1 || namespace == null || XmlUtil.EMPTY_URI.equals(namespace)) {
  ...
  private String getNsLocation(String ns) {
    ...
    if (XmlNSDescriptorImpl.equalsToSchemaName(this, XmlNSDescriptorImpl.SCHEMA_TAG_NAME)) {
      for (XmlTag subTag : getSubTags()) {
        if (XmlNSDescriptorImpl.equalsToSchemaName(subTag, XmlNSDescriptorImpl.IMPORT_TAG_NAME) && ns.equals(subTag.getAttributeValue("namespace"))) {
  ...
  private TreeElement addInternal(TreeElement child, ASTNode anchor, boolean before) throws IncorrectOperationException {
    ...
    if (child.getElementType() == XmlElementType.XML_ATTRIBUTE) {
      action = new InsertAttributeTransaction(child, anchor, before);
    } else if (anchor == null) {
      action = getBodyInsertTransaction(child);
    } else {
      action = new GenericInsertTransaction(child, anchor, before);
    }
    return action.getFirstInserted();
  }

  protected InsertAction getBodyInsertTransaction(final TreeElement child) {
  ...
  protected class BodyInsertTransaction implements InsertAction {
  ...
  protected class InsertAttributeTransaction implements InsertAction {
    private final TreeElement myChild;
    private final ASTNode myAnchor;
    private final boolean myBefore;

    public InsertAttributeTransaction(final TreeElement child, final ASTNode anchor, final boolean before) {
      myChild = child;
      myAnchor = anchor;
      myBefore = before;
    }
  ...
  protected class GenericInsertTransaction implements InsertAction {
    private final TreeElement myChild;
    private final ASTNode myAnchor;
    private final boolean myBefore;
    ...
    @Override
    public TreeElement getFirstInserted() {
      return XmlTagImpl.super.addInternal(myChild, myChild, myAnchor, Boolean.valueOf(myBefore));
    }
  }

  protected interface InsertAction {
