...
public class CreateNSDeclarationIntentionFix implements HintAction, LocalQuickFix {
    ...
    protected CreateNSDeclarationIntentionFix(
    ...
    public CreateNSDeclarationIntentionFix(
        @Nonnull final PsiElement element,
    ...
    public boolean showHint(@Nonnull final Editor editor) {
        ...
        if (!namespaces.isEmpty()) {
            final String message =
                AutoImportHelper.getInstance(element.getProject()).getImportMessage(namespaces.size() > 1, namespaces.iterator().next());
    ...
    public static void runActionOverSeveralAttributeValuesAfterLettingUserSelectTheNeededOne(
        @Nonnull final String[] namespacesToChooseFrom,
    ...
    private static void processExternalUrisImpl(
        final MetaHandler metaHandler,
        final PsiFile file,
        final ExternalUriProcessor processor
    ) {
