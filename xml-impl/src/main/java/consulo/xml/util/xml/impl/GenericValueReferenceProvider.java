  ...
  public final PsiReference[] getReferencesByElement(@Nonnull PsiElement psiElement, @Nonnull final ProcessingContext context) {
    ...
    if (psiElement instanceof XmlTag) {
      for (XmlText text : ((XmlTag) psiElement).getValue().getTextElements()) {
        if (injectedLanguageManager.getInjectedPsiFiles(text) != null) return PsiReference.EMPTY_ARRAY;
      }
    } else {
      if (injectedLanguageManager.getInjectedPsiFiles(psiElement) != null) return PsiReference.EMPTY_ARRAY;
    }
