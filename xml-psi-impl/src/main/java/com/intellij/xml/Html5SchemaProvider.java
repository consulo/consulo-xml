...
public abstract class Html5SchemaProvider {
    ...
    private synchronized static void ensureInitialized() {
        ...
        HTML5_SCHEMA_LOCATION = VirtualFileUtil.urlToPath(VirtualFileUtil.fixURLforIDEA(URLUtil.unescapePercentSequences(htmlSchemaLocationURL.toExternalForm())));
        LOG.info("HTML5_SCHEMA_LOCATION = " + getHtml5SchemaLocation());

        XHTML5_SCHEMA_LOCATION = VirtualFileUtil.urlToPath(VirtualFileUtil.fixURLforIDEA(URLUtil.unescapePercentSequences(xhtmlSchemaLocationURL.toExternalForm())));
        LOG.info("XHTML5_SCHEMA_LOCATION = " + getXhtml5SchemaLocation());

        CHARS_DTD_LOCATION = VirtualFileUtil.urlToPath(VirtualFileUtil.fixURLforIDEA(URLUtil.unescapePercentSequences(dtdCharsLocationURL.toExternalForm())));
