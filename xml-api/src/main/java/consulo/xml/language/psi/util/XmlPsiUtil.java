 ...
 */
package consulo.xml.language.psi.util;
...
public class XmlPsiUtil {
    ...
    public static boolean processXmlElementChildren(final XmlElement element, final PsiElementProcessor processor, final boolean deepFlag) {
        ...
        for (PsiElement child = element.getFirstChild(); child != null; child = child.getNextSibling()) {
            if (!p.processElement(child, deepFlag, wideFlag, true) && !wideFlag) {
                return false;
            }
        }
    ...
    private static class XmlElementProcessor {
        ...
        private boolean processXmlElements(PsiElement element, boolean deepFlag, boolean wideFlag, boolean processIncludes) {
            ...
            if (element instanceof XmlEntityRef ref) {
                if (!visitedEntities.add(ref.getText())) {
                    return true;
                }
                PsiElement newElement = parseEntityRef(targetFile, ref);

                while (newElement != null) {
                    if (!processElement(newElement, deepFlag, wideFlag, processIncludes)) {
                        return false;
                    }
                ...
                if (!xmlConditionalSection.isIncluded(targetFile)) {
                    return true;
                }
                startFrom = xmlConditionalSection.getBodyStart();
            }
            else if (processIncludes && XmlIncludeHandler.isXInclude(element)) {
                XmlInternal xmlInternal = targetFile.getProject().getApplication().getInstance(XmlInternal.class);

                for (PsiElement psiElement : xmlInternal.getIncludedTags((XmlTag)element)) {
                    if (!processElement(psiElement, deepFlag, wideFlag, true)) {
                        return false;
                    }
                }
            }
            ...
            for (PsiElement child = startFrom; child != null; child = child.getNextSibling()) {
                if (!processElement(child, deepFlag, wideFlag, processIncludes) && !wideFlag) {
                    return false;
                }
            }
    ...
    private static PsiElement parseEntityRef(PsiFile targetFile, XmlEntityRef ref) {
...

        {
            ...
            if (entityDecl != null) {
                return parseEntityDecl(entityDecl, targetFile, type, ref);
            }
        }
        ...
        if (element instanceof XmlFile xmlFile) {
            final XmlEntityDecl entityDecl = ref.resolve(xmlFile);
            if (entityDecl != null) {
                return parseEntityDecl(entityDecl, targetFile, type, ref);
            }
        }
    ...
    private static PsiElement parseEntityDecl(
        final XmlEntityDecl entityDecl,
        ...
        if (value == null) {
            value = CachedValuesManager.getManager(entityDecl.getProject()).createCachedValue(
                () -> {
                    ...
                    return new CachedValueProvider.Result<>(
                        res,
                        res.getUserData(XmlElement.DEPENDING_ELEMENT),
                        entityDecl,
                        targetFile,
                        entityRef
                    );
            ...
            value = entityRef.putUserDataIfAbsent(PARSED_DECL_KEY, value);
        }
